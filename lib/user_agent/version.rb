class UserAgent
  class Version
    ...
    def <=>(other)
      ...
      when Version
        if @comparable
          list_a, list_b = to_a, other.to_a
          max_len = [list_a.size, list_b.size].max

          # zip truncates if the source list is shorter than its args, so iterate over our max length.
          (0...max_len).zip(list_a, list_b).each do |i, a, b|
            a ||= 0
            b ||= 0
